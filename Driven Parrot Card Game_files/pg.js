...
for(let index = 0; index < card_quantity; index++){
    board.innerHTML += 
    ` 
    <div class="card" onclick="desvira(this)">
...
function desvira(elemento){
    elemento.querySelector('.front-face').classList.toggle('select-front');
    elemento.querySelector('.back-face').classList.toggle('select-back');
